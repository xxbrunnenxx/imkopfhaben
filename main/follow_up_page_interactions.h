#ifndef FOLLOW_UP_PAGE_INTERACTIONS_H_
#define FOLLOW_UP_PAGE_INTERACTIONS_H_
...
// Primary (OK / tap-release) on a focused date chip enters its item list; on a focused follow-up
// it asks to open the item-actions modal; on the footer it routes to a page.
ActivateResult HandlePrimaryActivate(FollowUpPageCoordinator& coordinator);
void ApplyPrimaryActivateResult(const ActivateResult& result, const ActivateCallbacks& callbacks);
FocusMoveResult HandleMoveFocus(FollowUpPageCoordinator& coordinator, int delta);

}  // namespace follow_up_page_interactions

#endif  // FOLLOW_UP_PAGE_INTERACTIONS_H_
