...
| Ueber-Limit wird markiert, nicht gesperrt | Route bei Stand 6 lesen (Ziel 4) | belegt (Route) — `today_count` 6 > goal 4, Route liefert weiter statt zu deckeln. Die Punktdarstellung auf dem Display (6 Punkte, die 2 ueber Ziel mit hellem Kern) ist Code (`joint_tracker_card.cpp`), aber **nicht am Schirm beobachtet** | 20.09. |
| Up/Down zaehlt am Geraet mit Kontrollton | Am Geraet: hoch zum 420-Track, Klick, Up/Down | **teilbelegt** — Route zeigte drei getrennte Uebergaenge nach oben (5→6→7) ueber die Sitzung, dazwischen 45 s unberuehrt konstant (kein Auto-Increment). Der Pfad Taste→+1→NVS→Route ist damit indirekt belegt; die Tastendruecke kamen vom Besitzer am Board, nicht von mir, und die Kontrollton-/Fokus-Optik ist noch nicht bestaetigt | 20.09. |
